...
// Check if the Finish Process button is clicked
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['finishButton'])) {
    // Get the IDs of the selected orders
    if (!empty($_POST['selectedOrders'])) {
        $selectedOrders = implode(',', $_POST['selectedOrders']);

        // Update the order status to 'Finish Process' for the selected orders
        $updateSql = "UPDATE pos SET orderstatus = 'Finish Process' WHERE id IN ($selectedOrders)";
...
$sql = "SELECT id, customer_tag, pizzaname, price, quantity, orderstatus FROM pos WHERE user = '$username' AND orderstatus NOT IN ('Pending', 'Finish Process')";
...
<div class="container mt-3">
  ...
  <div class="container">
    <div class="table-responsive">
      ...
      <table class="table table-bordered">
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            if ($row['orderstatus'] == 'Cooking') {
                $statusClass = 'cooking-text'; // Set the class for styling
            }
      ...
      <button type="submit" name="finishButton" class="btn btn-primary">Finish Process</button> <!-- Functional button outside the table -->
      </form>
    </div>
  </div>
</div>
